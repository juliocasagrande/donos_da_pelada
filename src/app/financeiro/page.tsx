...
export default async function FinancePage({
  ...
  const historyRows = [...history.entries()]
  ...
  return (
    <AppShell>
      ...
      <div className="mb-4 flex items-center justify-between gap-2 rounded-[13px] bg-white px-2 py-2 shadow-card">
        <Link
          href={`/financeiro?mes=${monthKey(prevMonth.year, prevMonth.month)}`}
        ...
        <Link
          href={`/financeiro?mes=${monthKey(nextMonth.year, nextMonth.month)}`}
          className="flex h-9 w-9 items-center justify-center rounded-[10px] text-musgo"
        >
      ...
      <div className="mb-2 flex items-center justify-between">
        <h2 className="font-display text-lg font-extrabold">Mensalistas</h2>
        <span className="text-sm text-musgo">
          {paidPlayerIds.size}/{players.length} pagaram
        </span>
      </div>
      ...
      <Card className="mb-4">
        <h2 className="mb-3 font-display text-lg font-extrabold">Novo lancamento avulso</h2>
        <form action={createTransaction} className="space-y-3">
          ...
          <Button type="submit" className="w-full">Adicionar lancamento</Button>
        </form>
      </Card>

      <h2 className="mb-2 font-display text-lg font-extrabold">Lancamentos do mes</h2>
      <div className="mb-4 space-y-2">
      ...
      <h2 className="mb-2 font-display text-lg font-extrabold">Historico mensal</h2>
