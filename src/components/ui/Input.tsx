...
export function Input({ className, ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        "min-h-10 w-full min-w-0 max-w-full rounded-[13px] border-[1.5px] border-linha bg-[#F6F8F3] px-3.5 py-2 text-tinta outline-none placeholder:text-musgo/60 focus:border-campo",
        className
      )}
      {...props}
    />
