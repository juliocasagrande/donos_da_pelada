...
export async function assertMatchInPelada(matchId: string, peladaId: string) {
  const match = await prisma.match.findFirst({ where: { id: matchId, peladaId }, select: { id: true } });
...
export async function assertPollInPelada(pollId: string, peladaId: string) {
  const poll = await prisma.poll.findFirst({ where: { id: pollId, match: { peladaId } }, select: { id: true, matchId: true } });
